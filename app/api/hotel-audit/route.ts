...
async function generateQuestions(ctx: { name: string; city: string; type: string; pages: string[]; amenities: string[]; demand?: any; regionQueries?: string[] }, openaiKey: string): Promise<any[]> {
  ...
  const regionBlock = (ctx.regionQueries && ctx.regionQueries.length)
    ? `\nREAL AI-SEARCH QUERIES PEOPLE ASK IN THIS REGION (source of truth):\n${ctx.regionQueries.map((q, i) => `${i + 1}. ${q}`).join('\n')}\n\nQUESTION MIX — THIS IS THE MOST IMPORTANT RULE:\n- The MAJORITY of your questions (aim ~60%) MUST be DISCOVERY questions: comparative "best X in ${'${ctx.city}'}"-style prompts phrased exactly like the region queries above (e.g. "Is [hotel] the best spa hotel in ${'${ctx.city}'}?", "What are the best romantic hotels in ${'${ctx.city}'}?"). Cover EACH region-query theme above with at least one discovery question, collapsing near-duplicates so each theme appears ONCE.\n- Keep EVALUATION questions (self-referential "does the hotel have X") to a MINIMUM — only where no discovery framing fits.\n- Include ~5 BOOKING questions (parking/EV, pets, accessibility, airport transfer, cancellation/check-in).\nDiscovery questions are what win AI visibility, so they must dominate the set, not evaluation.\n`
    : ''
