...
export async function generateStaticParams() {
  ...
  for (const hotel of hotels || []) {
    for (const intent of Object.keys(INTENTS)) {
      params.push({ slug: (hotel as any).slug || hotel.id, intent })
    }
  }
  return params
}
...
export default async function IntentPage({ params }: { params: Promise<{ slug: string; intent: string }> }) {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: hotelData.name, item: `https://swissnethotels.com/hotels/${hotelUrl}` },
      { '@type': 'ListItem', position: 4, name: intentData.label, item: `https://swissnethotels.com/hotels/${hotelUrl}/${intent}` },
    ]
  }
      ...
      <div style={{ position: 'relative', height: '45vh', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', bottom: '2rem', left: 0, right: 0, maxWidth: '1200px', margin: '0 auto', padding: '0 2rem' }}>
          ...
          <p style={{ fontFamily: 'Montserrat, sans-serif', fontSize: '0.75rem', color: 'rgba(255,255,255,0.8)', margin: 0 }}>
            {hotelData.location} · ★ {hotelData.rating} · From CHF {hotelData.nightly_rate_chf?.toLocaleString()}/night
          </p>
        </div>
      </div>
        ...
        <div>
          ...
          <div style={{ marginBottom: '2.5rem' }}>
            <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.8rem', fontWeight: 300, color: text, marginBottom: '1rem' }}>
              {hotelData.name} for {intentData.label} — FAQs
            </h2>
          ...
          <div style={{ marginBottom: '2.5rem' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
              {Object.entries(INTENTS).filter(([k]) => k !== intent).map(([k, v]) => (
                <Link key={k} href={`/hotels/${hotelUrl}/${k}`} style={{ fontFamily: 'Montserrat, sans-serif', fontSize: '0.65rem', color: text, border: '1px solid ' + border, padding: '0.35rem 0.875rem', background: '#fff', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                  <span style={{ color: gold }}>✦</span> {hotelData.name} for {v.label}
                </Link>
              ))}
            </div>
          </div>

          <Link href={`/hotels/${hotelUrl}`} style={{ fontFamily: 'Montserrat, sans-serif', fontSize: '0.7rem', color: textMuted, textDecoration: 'none' }}>
            ← Back to {hotelData.name}
          </Link>
        </div>
        ...
        <div>
          <div style={{ background: '#fff', border: `1px solid ${gold}88`, padding: '1.5rem', position: 'sticky', top: '2rem' }}>
            ...
            <div style={{ textAlign: 'center' }}>
              <Link href={`/hotels/${hotelUrl}`} style={{ fontFamily: 'Montserrat, sans-serif', fontSize: '0.7rem', color: gold, textDecoration: 'none' }}>
                View full profile →
              </Link>
            </div>
          </div>
