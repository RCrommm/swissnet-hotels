...
const PRESET_QUERIES = [
  'best luxury hotel Geneva',
  'best spa hotel Geneva',
  'best romantic hotel Geneva',
  'best hotel on Lake Geneva',
  'best 5 star hotel Geneva',
  'best boutique hotel Geneva',
  'best hotel for couples Geneva',
  'best lakeside hotel Geneva',
  'luxury hotel with spa Geneva',
  'best hotel lake view Geneva',
  'most exclusive hotel Geneva',
  'best wellness hotel Geneva',
  'best fine dining hotel Geneva',
  'best hotel for honeymoon Geneva',
  'top luxury hotel Geneva',
]
...
export default function GoogleAITab({ hotels }: { hotels: any[] }) {
  ...
  const allAnswered = answered === PRESET_QUERIES.length
  const score = allAnswered
    ? Math.round((Object.values(results).filter(Boolean).length / PRESET_QUERIES.length) * 100)
    : null
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    setError('')
    const promises = PRESET_QUERIES.map(query =>
          ...
          <>
            <div style={{ marginBottom: 16, display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ flex: 1, height: 4, background: '#f5f5f4', borderRadius: 2 }}>
                <div style={{ height: 4, background: '#C9A84C', borderRadius: 2, width: `${(answered / PRESET_QUERIES.length) * 100}%`, transition: 'width 0.3s' }} />
              </div>
              <span style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 11, color: '#78716c', whiteSpace: 'nowrap' }}>{answered} / {PRESET_QUERIES.length}</span>
            </div>
            ...
            {allAnswered && (
              <div style={{ background: '#fafaf9', border: '1px solid #e7e5e4', borderRadius: 6, padding: '12px 16px', marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 13, color: '#78716c' }}>Google AI Score</span>
