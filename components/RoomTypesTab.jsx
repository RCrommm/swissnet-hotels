// components/admin/RoomTypesTab.jsx
// Full room type management tab for the SwissNet admin panel
// Drop into your admin panel alongside the existing tabs
// Usage: <RoomTypesTab hotelId={selectedHotelId} hotelName={selectedHotelName} />
...
const AMENITY_PRESETS = [
  ...
  'Blackout Curtains', 'Soundproofing', 'Connecting Room Option',
];
...
export default function RoomTypesTab({ hotelId, hotelName }) {
  const [rooms, setRooms] = useState([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [editing, setEditing] = useState(null); // null | 'new' | room object
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      price_peak: form.price_peak ? parseFloat(form.price_peak) : null,
      ...
      min_stay_nights: parseInt(form.min_stay_nights) || 1,
    ...
    if (editing === 'new') {
      ...
      ({ error } = await supabase.from('room_types').update(payload).eq('id', editing.id));
    }
  ...
  const S = {
    ...
    btnGold: {
      background: COLORS.gold, color: '#1a0e06', border: 'none',
      padding: '10px 22px', borderRadius: 8, fontWeight: 700, fontSize: 14,
      cursor: 'pointer', transition: 'opacity .15s'
    },
    btnOutline: {
      background: 'transparent', color: COLORS.gold,
      border: `1px solid ${COLORS.gold}`, padding: '8px 18px',
      borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: 'pointer'
    },
    btnDanger: {
      background: 'transparent', color: COLORS.danger,
      border: `1px solid ${COLORS.danger}`, padding: '6px 14px',
    ...
    // Form
    formWrap: {
      background: COLORS.bgCard, border: `1px solid ${COLORS.gold}55`,
  ...
  if (!hotelId) return (
    <div style={S.empty}>Select a hotel to manage room types.</div>
  );
  ...
  return (
    <div style={S.wrap}>
      ...
      {editing && (
        <div style={S.formWrap}>
          <div style={S.formTitle}>
            {editing === 'new' ? '✦ New Room Type' : `✦ Edit: ${editing.name}`}
          </div>
          ...
          <div style={S.grid2}>
            <div style={S.fieldGroup}>
              ...
              <input {...F('name')} placeholder="e.g. Deluxe King Room" />
            </div>
            <div style={S.fieldGroup}>
              <label style={S.label}>Category</label>
              <select style={S.select} value={form.type_category}
                onChange={e => setForm(f => ({ ...f, type_category: e.target.value }))}>
          ...
          <div style={{ ...S.fieldGroup, marginBottom: 16 }}>
            <label style={S.label}>Short Description (shown in schema / AI snippets)</label>
            <input {...F('short_description')} placeholder="One-line description for AI crawlers and snippets" />
          </div>

          <div style={{ ...S.fieldGroup, marginBottom: 16 }}>
            <label style={S.label}>Full Description</label>
            <textarea {...F('description')} style={S.textarea}
              placeholder="Detailed room description for the hotel profile page..." />
          </div>

          <div style={{ ...S.fieldGroup, marginBottom: 16 }}>
            <label style={S.label}>AI-Optimised Description (for schema.org / chatbot visibility)</label>
            <textarea {...F('ai_description')} style={{ ...S.textarea, borderColor: COLORS.gold + '55' }}
              placeholder="Optimised description written specifically for AI crawler indexing. Include key selling points, amenity keywords, and location context." />
          </div>
          ...
          <div style={S.grid3}>
            <div style={S.fieldGroup}>
              ...
              <input {...F('price_per_night')} type="number" placeholder="850" />
            </div>
            <div style={S.fieldGroup}>
              <label style={S.label}>Weekend Rate</label>
              <input {...F('price_weekend')} type="number" placeholder="optional" />
            </div>
            <div style={S.fieldGroup}>
              <label style={S.label}>Peak Season Rate</label>
              <input {...F('price_peak')} type="number" placeholder="optional" />
            </div>
          </div>
          <div style={S.grid2}>
            <div style={S.fieldGroup}>
              <label style={S.label}>Currency</label>
              <select style={S.select} value={form.price_currency}
            ...
            <div style={S.fieldGroup}>
              ...
              <input {...F('min_stay_nights')} type="number" min="1" />
            </div>
          </div>
          ...
          <div style={S.grid3}>
            <div style={S.fieldGroup}>
              ...
              <input {...F('size_sqm')} type="number" placeholder="45" />
            </div>
            <div style={S.fieldGroup}>
              <label style={S.label}>Bed Type</label>
              <select style={S.select} value={form.bed_type}
            ...
            <div style={S.fieldGroup}>
              ...
              <input {...F('bed_count')} type="number" min="1" max="6" />
            </div>
          </div>
          <div style={S.grid3}>
            <div style={S.fieldGroup}>
              <label style={S.label}>View Type</label>
              <select style={S.select} value={form.view_type}
            ...
            <div style={S.fieldGroup}>
              ...
              <input {...F('floor_level')} placeholder="e.g. 3rd–5th" />
            </div>
            <div style={S.fieldGroup}>
              <label style={S.label}>Max Occupancy</label>
              <input {...F('max_occupancy')} type="number" min="1" max="20" />
            </div>
          </div>
          ...
          <div style={S.tagWrap}>
            {AMENITY_PRESETS.map(a => (
              <span key={a} style={S.tag(form.amenities.includes(a))}
                onClick={() => toggleAmenity(a)}>{a}</span>
            ))}
          </div>
          {/* Custom amenity */}
          <div style={S.tagInput}>
            <input style={{ ...S.input, flex: 1 }} value={amenityInput}
              onChange={e => setAmenityInput(e.target.value)}
              placeholder="Add custom amenity..."
              onKeyDown={e => e.key === 'Enter' && addTag('amenities', amenityInput, setAmenityInput)} />
            <button style={S.btnOutline}
              onClick={() => addTag('amenities', amenityInput, setAmenityInput)}>Add</button>
          </div>

          {/* Highlights */}
          <div style={S.sectionTitle}>Highlights (top 3–5 AI selling points)</div>
          <div style={S.tagWrap}>
            {form.highlights.map(h => (
              <span key={h} style={{ ...S.tag(true), cursor: 'default' }}>
                {h} <span style={{ cursor: 'pointer', marginLeft: 4 }}
                  onClick={() => removeTag('highlights', h)}>×</span>
              </span>
            ))}
          </div>
          <div style={S.tagInput}>
            <input style={{ ...S.input, flex: 1 }} value={highlightInput}
              onChange={e => setHighlightInput(e.target.value)}
              placeholder="e.g. Panoramic Matterhorn views..."
              onKeyDown={e => e.key === 'Enter' && addTag('highlights', highlightInput, setHighlightInput)} />
            <button style={S.btnOutline}
              onClick={() => addTag('highlights', highlightInput, setHighlightInput)}>Add</button>
          </div>
          ...
          <div style={S.tagWrap}>
            {form.keywords.map(k => (
              <span key={k} style={{ ...S.tag(false), color: COLORS.textMuted }}>
                {k} <span style={{ cursor: 'pointer', marginLeft: 4 }}
                  onClick={() => removeTag('keywords', k)}>×</span>
              </span>
            ))}
          </div>
          <div style={S.tagInput}>
            <input style={{ ...S.input, flex: 1 }} value={keywordInput}
              ...
              placeholder="e.g. luxury suite zermatt matterhorn view..."
              onKeyDown={e => e.key === 'Enter' && addTag('keywords', keywordInput, setKeywordInput)} />
            <button style={S.btnOutline}
              onClick={() => addTag('keywords', keywordInput, setKeywordInput)}>Add</button>
          </div>
          ...
          <div style={S.grid2}>
            <div style={S.fieldGroup}>
              <label style={S.label}>Booking URL (deep link)</label>
              <input {...F('booking_url')} placeholder="https://..." />
            </div>
            <div style={S.fieldGroup}>
              <label style={S.label}>Thumbnail URL</label>
              <input {...F('thumbnail_url')} placeholder="https://..." />
            </div>
          </div>

          {/* Schema / Advanced */}
          <div style={S.sectionTitle}>Schema / Advanced</div>
          <div style={S.grid2}>
            <div style={S.fieldGroup}>
              <label style={S.label}>Schema Name Override (optional)</label>
              <input {...F('schema_name')} placeholder="Leave blank to use room name" />
            </div>
            <div style={S.fieldGroup}>
              <label style={S.label}>Sort Order</label>
              <input {...F('sort_order')} type="number" />
            </div>
          </div>
          <div style={{ ...S.checkWrap, marginTop: 12 }}>
            ...
            <label htmlFor="is_available" style={{ color: COLORS.textMuted, fontSize: 14, cursor: 'pointer' }}>
              Room is available (shown in schema &amp; listing)
            </label>
          </div>
          ...
          <div style={S.actions}>
            ...
            <button style={{ ...S.btnGold, opacity: saving ? 0.6 : 1 }}
      ...
      {!editing && (
        <>
          {loading && <div style={S.empty}>Loading room types...</div>}
          {!loading && rooms.length === 0 && (
            <div style={S.empty}>
              No room types yet for {hotelName}.<br />
              <span style={{ color: COLORS.gold, cursor: 'pointer', marginTop: 8, display: 'inline-block' }}
                onClick={openNew}>+ Add your first room type →</span>
            </div>
          )}
          {rooms.map(room => (
            <div key={room.id} style={S.card}>
              ...
              <div style={{
                ...
                background: room.is_available ? COLORS.success : COLORS.danger
              }} />
              <div style={S.cardLeft}>
                <div style={S.roomName}>{room.name}</div>
                <div style={S.roomMeta}>
                  <span style={S.badge(COLORS.gold)}>{room.type_category}</span>
                  {room.price_per_night && (
                    <span>CHF {parseFloat(room.price_per_night).toLocaleString()} / night</span>
                  )}
                  {room.size_sqm && <span>{room.size_sqm} m²</span>}
                  {room.bed_type && <span>{room.bed_count || 1}× {room.bed_type}</span>}
                  {room.view_type && <span>{room.view_type}</span>}
                  {room.max_occupancy && <span>Max {room.max_occupancy} guests</span>}
                  {room.amenities?.length > 0 && (
                    <span style={{ color: COLORS.gold }}>{room.amenities.length} amenities</span>
                  )}
                </div>
              </div>
