      ...
      <div style={{ background: bgLight, paddingTop: '6rem', paddingBottom: '3rem', borderBottom: '1px solid ' + border }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 2rem' }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid ' + border, borderRadius: 12, padding: '1.25rem', display: 'flex', gap: '0.75rem', flexWrap: 'wrap' as const, alignItems: 'flex-end' }}>
            ...
            <button onClick={handleSearch} style={{ background: gold, color: '#1a0e06', fontFamily: 'Montserrat, sans-serif', fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase' as const, padding: '0.65rem 1.5rem', border: 'none', borderRadius: 8, cursor: 'pointer', whiteSpace: 'nowrap' as const }}>
              Search
            </button>
          </div>
        </div>
            ...
            {hotels.map(hotel => {
              ...
              return (
                <div
                  ...
                  style={{
                    background: '#FFFFFF',
                    border: hotel.is_partner ? `1px solid ${gold}99` : '1px solid rgba(201,169,110,0.2)',
                    borderRadius: 20,
                    overflow: 'hidden',
                    transition: 'all 0.35s ease',
                    transform: isHovered ? 'translateY(-6px)' : 'translateY(0)',
                    boxShadow: isHovered ? '0 20px 56px rgba(0,0,0,0.22)' : '0 2px 16px rgba(0,0,0,0.08)',
                    position: 'relative',
                  }}
                >
                  {/* Clickable overlay */}
                  ...
                  <div style={{ position: 'relative', height: '230px', overflow: 'hidden' }}>
                    ...
                    {/* Partner badge */}
                    {hotel.is_partner && (
                      <div style={{ position: 'absolute', top: '1rem', left: '1rem', background: gold, color: '#1a0e06', fontFamily: 'Montserrat, sans-serif', fontSize: '0.5rem', fontWeight: 700, letterSpacing: '0.15em', textTransform: 'uppercase' as const, padding: '0.3rem 0.875rem', borderRadius: 20, zIndex: 2 }}>
                        ✦ Partner
                      </div>
                    )}
                  ...
                  <div style={{ padding: '1.25rem 1.375rem' }}>
                    ...
                    {/* Location + guest rating */}
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                        <svg width="11" height="11" fill="none" stroke={gold} viewBox="0 0 24 24">
                    ...
                    {/* Description */}
                    <p style={{ fontFamily: 'Montserrat, sans-serif', fontSize: '0.7rem', color: 'rgba(61,43,31,0.55)', lineHeight: 1.7, marginBottom: '1rem', overflow: 'hidden', display: '-webkit-box' as const, WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' as const }}>
                      {hotel.description}
                    </p>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: '1rem', borderTop: '1px solid rgba(201,169,110,0.15)' }}>
                      <div>
                        ...
                        <p style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.45rem', fontWeight: 400, color: '#3D2B1F', margin: 0 }}>
                          CHF {hotel.nightly_rate_chf.toLocaleString()}
                          <span style={{ fontFamily: 'Montserrat, sans-serif', fontSize: '0.55rem', color: 'rgba(61,43,31,0.4)', fontWeight: 300 }}> /night</span>
                        </p>
                      </div>
                      <div style={{ display: 'flex', gap: '0.5rem', position: 'relative', zIndex: 2 }}>
                          ...
                          style={{ fontFamily: 'Montserrat, sans-serif', fontSize: '0.58rem', fontWeight: 500, letterSpacing: '0.12em', textTransform: 'uppercase' as const, color: '#3D2B1F', border: '1px solid rgba(201,169,110,0.35)', borderRadius: 8, padding: '0.6rem 0.875rem', textDecoration: 'none' }}
                        >
                          Website
                        </a>
                          ...
                          target="_blank" rel="noopener noreferrer"
                          style={{ fontFamily: 'Montserrat, sans-serif', fontSize: '0.58rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase' as const, color: '#1a0e06', background: gold, borderRadius: 8, padding: '0.6rem 0.875rem', textDecoration: 'none' }}
                        >
                          Book Direct
                        </a>
                      </div>
