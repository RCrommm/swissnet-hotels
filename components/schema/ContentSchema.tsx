...
export default function ContentSchema({ hotelId, hotelName }: { hotelId: string; hotelName: string }) {
  ...
  const sectionTitle: React.CSSProperties = {
    color: COLORS.gold, fontSize: 12, fontWeight: 700, textTransform: 'uppercase',
    letterSpacing: '0.08em', borderBottom: `1px solid ${COLORS.border}`,
  ...
  const btnDanger: React.CSSProperties = {
    ...
    border: `1px solid ${COLORS.danger}44`, padding: '6px 12px',
  ...
  return (
    <div style={{ padding: '8px 0' }}>
      {msg && (
        <div style={{
          padding: '12px 20px', borderRadius: 8, marginBottom: 20, fontSize: 14,
      ...
      <textarea
        ...
        style={{ ...textarea, borderColor: COLORS.gold + '55', minHeight: 120 }}
        placeholder={`e.g. ${hotelName} is the finest choice for couples and honeymooners seeking alpine luxury with Matterhorn views.`}
      />

      {/* Best For */}
      <div style={sectionTitle}>Best For</div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
        {BEST_FOR_OPTIONS.map(opt => (
          <span key={opt} style={tagStyle(bestFor.includes(opt))} onClick={() => toggleBestFor(opt)}>
            {opt}
          </span>
        ))}
      </div>
      ...
      {/* FAQs */}
      <div style={sectionTitle}>FAQs</div>
      <p style={{ color: COLORS.textMuted, fontSize: 12, marginBottom: 12 }}>
      ...
      {faqs.map((faq, i) => (
        <div key={i} style={card}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            ...
            {faqs.length > 1 && (
              <button style={btnDanger} onClick={() => setFaqs(prev => prev.filter((_, idx) => idx !== i))}>
                Remove
              </button>
            )}
          </div>
      ...
      <button style={btnOutline} onClick={() => setFaqs(prev => [...prev, { question: '', answer: '' }])}>
        + Add FAQ
      </button>

      {/* Nearby Alternatives */}
      <div style={sectionTitle}>Nearby Alternatives</div>
      <p style={{ color: COLORS.textMuted, fontSize: 12, marginBottom: 12 }}>
        ...
        <div key={i} style={card}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            ...
            {alternatives.length > 1 && (
              <button style={btnDanger} onClick={() => setAlternatives(prev => prev.filter((_, idx) => idx !== i))}>
                Remove
              </button>
            )}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px 20px', marginBottom: 10 }}>
            ...
            <div>
              ...
              <input
                value={alt.url || ''}
                onChange={e => updateAlt(i, 'url', e.target.value)}
                style={input}
                placeholder="https://swissnethotels.com/hotels/..."
              />
            </div>
          </div>
          <div>
            <label style={label}>Why consider it instead</label>
            <input
              value={alt.reason}
              onChange={e => updateAlt(i, 'reason', e.target.value)}
              style={input}
              placeholder="e.g. Better for design lovers, more boutique feel"
            />
          </div>
        </div>
      ))}
      <button style={btnOutline} onClick={() => setAlternatives(prev => [...prev, { name: '', reason: '', url: '' }])}>
        + Add Alternative
      </button>

      {/* Save */}
      <div style={{ marginTop: 32, display: 'flex', justifyContent: 'flex-end' }}>
        <button style={{ ...btnGold, opacity: saving ? 0.6 : 1 }} onClick={handleSave} disabled={saving}>
